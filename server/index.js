...
const mongoUri = process.env.MONGODB_URI || "mongodb://127.0.0.1:27017/palina_store";
...
app.use(cookieParser(process.env.COOKIE_SECRET || "palina-dev-cookie-secret-change-me"));
...
app.get("/api/health", (_req, res) => {
  res.json({ ok: true, service: "palina-store-api" });
});
...
const start = async () => {
  ...
  app.listen(port, "0.0.0.0", () => {
    console.log(`Palina API listening on http://localhost:${port}`);
  });
};
